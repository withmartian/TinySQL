...
class TestAblateBM1(unittest.TestCase):
    ...
    def setUp(self):
        ...
        collect_bm1_activations(self.model, self.tokenizer, inputs)
        #print(f"Max Length: Prompts {len(prompts[0])}, Inputs {len(inputs[0])}")
 
    def test_ablate_bm1_none(self):
        batch_items = generate_cs1(100)
        (_, inputs) = generate_inputs_from_BatchItems(self.tokenizer, batch_items)
        (_, generated_text) = ablate_bm1(self.tokenizer, self.model, inputs, node_type="", layer_index=0)
        print(f"No Hook: {generated_text[0]}")

    def test_ablate_bm1_layer(self):
        batch_items = generate_cs1(100)
        (_, inputs) = generate_inputs_from_BatchItems(self.tokenizer, batch_items)        
        (_, generated_text) = ablate_bm1(self.tokenizer, self.model, inputs, node_type="layer", layer_index=0)
        (_, generated_text) = ablate_bm1(self.tokenizer, self.model, inputs, node_type="layer", layer_index=1)
        #print(f"Layer Hook: {generated_text[0]}")

    def test_ablate_bm1_mlp(self):
        batch_items = generate_cs1(100)
        (_, inputs) = generate_inputs_from_BatchItems(self.tokenizer, batch_items)        
        (_, generated_text) = ablate_bm1(self.tokenizer, self.model, inputs, node_type="mlp", layer_index=0)
        (_, generated_text) = ablate_bm1(self.tokenizer, self.model, inputs, node_type="mlp", layer_index=1)
        print(f"MLP Hook: {generated_text[0]}")

    def test_ablate_bm1_head(self):
        batch_items = generate_cs1(100)
        (_, inputs) = generate_inputs_from_BatchItems(self.tokenizer, batch_items)        
        (_, generated_text) = ablate_bm1(self.tokenizer, self.model, inputs, node_type="head", layer_index=0, head_index=5)
        (_, generated_text) = ablate_bm1(self.tokenizer, self.model, inputs, node_type="head", layer_index=1, head_index=4)
