...
def collect_bm1_activations(model, tokenizer, batched_inputs):
    ...
    def collect_activations_hook(module, input, output, layer_index=None, head_index=None):
        ...
        if layer_index is not None:
            if head_index is not None:
                ...
                average_bm1_activations["layer"].append(output.mean(dim=0).detach().clone())
...
def ablate_bm1(tokenizer, model, batched_inputs, node_type="layer", layer_index=0, head_index=None):
    ...
    def ablation_hook(module, input, output):
        ...
        if node_type == "head" and head_index is not None:
            the_activation = average_bm1_activations["head"][layer_index][head_index].clone()
