...
def evaluate(
    ...
    tokenizer.pad_token = (
        tokenizer.eos_token
    )  # Set pad token to eos token if pad token is undefined
    ...
    for batch in tqdm(test_dataloader, desc="Evaluating"):
        ...
        batch_prediction_scores_tensor = torch.tensor(
            batch_prediction_scores, device=accelerator.device
        )
        batch_gt_scores_tensor = torch.tensor(
            batch_gt_scores, device=accelerator.device
        )
    ...
    assert num_samples == len(
        dataset
    ), "Number of samples processed should match the dataset size"
    ...
    try:
        ...
        def preprocess_function(examples):
            ...
            for prompt_ids, response_ids in zip(
                ...
                labels = [-100] * len(
                    prompt_ids
                ) + response_ids  # Mask prompt tokens in labels
                ...
                if args.debug:
                    ...
                    filtered_input_ids_labels = [
                        id for id, label in zip(input_ids, labels) if label != -100
                    ]
                    print(
                        "Decoded input_ids (where labels != -100):",
                        tokenizer.decode(filtered_input_ids_labels),
                    )
                    # Decode and print the filtered input_ids tokens where attention_mask is not 0
                    filtered_input_ids_attention = [
                        id for id, mask in zip(input_ids, attention_mask) if mask != 0
                    ]
                    print(
                        "Decoded input_ids (where attention_mask != 0):",
                        tokenizer.decode(filtered_input_ids_attention),
                    )
        ...
        train_dataset, val_dataset, test_dataset = accelerator.prepare(
        ...
        effective_batch_size = (
            args.batch_size * args.gradient_accumulation_steps * num_devices
        )
